...
export const TOOLS = [
  ...
  { name: 'metrc_create_package', description: 'Create a new package. Requires item_id, tag, quantity, unit, location_id, and optional label.', inputSchema: { type: 'object', properties: { license_number: { type: 'string' }, tag: { type: 'string' }, location_id: { type: 'number' }, item_id: { type: 'number' }, quantity: { type: 'number' }, unit_of_measure: { type: 'string' }, is_production_batch: { type: 'boolean' }, product_requires_remediation: { type: 'boolean' }, actual_date: { type: 'string' }, ingredients: { type: 'array' } }, required: ['license_number', 'tag', 'location_id', 'item_id', 'quantity', 'unit_of_measure', 'actual_date'] } },
