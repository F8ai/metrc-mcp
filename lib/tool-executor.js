...
async function executeToolCall(name, args, metrcFetch) {
  switch (name) {
    ...
    case 'metrc_submit_packages_for_testing': {
      const today = new Date().toISOString().split('T')[0];
      const body = (args.packages || []).map(p => ({
        Tag: p.tag,
        Location: null,
        Item: p.item_name,
        Quantity: p.quantity,
        ...
        SourcePackageLabels: p.source_package_label,
        Note: '',
      }));
      return metrcFetch(
        '/packages/v2/testing',
    ...
    case 'metrc_get_plant_batch_waste':
      return metrcFetch('/plantbatches/v2/waste', { licenseNumber: args.license_number });

    case 'metrc_get_plant_batch_waste_reasons':
      return metrcFetch('/plantbatches/v2/waste/reasons', { licenseNumber: args.license_number });

    case 'metrc_post_plant_batch_waste':
      return metrcFetch(
        ...
        { licenseNumber: args.license_number },
        {
          method: 'POST',
          body: [{
            PlantBatchId: args.plant_batch_id,
            PlantBatchName: args.plant_batch_name,
            WasteReasonName: args.waste_reason_name,
            WasteWeight: args.waste_weight,
            UnitOfWeightName: args.unit_of_weight,
            ActualDate: args.actual_date,
          }],
        }
      );

    case 'metrc_get_package_adjust_reasons':
      return metrcFetch('/packages/v2/adjust/reasons', { licenseNumber: args.license_number });

    case 'metrc_delete_harvest_waste':
      return metrcFetch(
        `/harvests/v2/waste/${args.waste_id}`,
        { licenseNumber: args.license_number },
        { method: 'DELETE' }
      );

    case 'metrc_post_destroy_plants':
      return metrcFetch(
        '/plants/v2/destroyplants',
        { licenseNumber: args.license_number },
        {
          method: 'POST',
          body: (args.plant_ids || []).map(id => ({
            Id: id,
            WasteMethodName: args.waste_method_name,
            ReasonNote: args.reason_note,
            ActualDate: args.actual_date,
          })),
        }
