...
**Published:** This content is published as the [Framework](framework.md) page in the GitHub Pages documentation. The web version is the canonical doc (with MCP/skills links); this file is the source.

Mapping of the product/feature framework to METRC MCP capabilities. **MCP** = can implement with current or new tools/resources. **Skill** = analysis pattern (LLM + tools). **App** = needs UI, rules engine, or external data.
...
This doc can be used as a roadmap: implement **skills** and **array-capable tools** first for high-feasibility items; reserve **App** for proprietary logic, GUI, and multi-source data.
